...
while True:
    ...
    if c == ord('f'):
        ...
        for j in range(W):
            ...
            if j == W-1:
                f_ = pysynth.make_wav(temp, fn = "fractal_song.wav", superimpose=True, close_f = True, f=f_)
            else:
                f_ = pysynth.make_wav(temp, fn = "fractal_song.wav", superimpose=True, close_f = False, f=f_)