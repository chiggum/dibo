...
def make_wav(song,bpm=120,transpose=0,pause=.05,boost=1.1,repeat=0,fn="out.wav", superimpose=False,  f = None, close_f = True, silent=False):
	if superimpose:
		last_ts = None
	...
	def render2(a,b,vol,glob_intensity_list=None):
		...
		if not superimpose:
			for x in range(q):
				ow = ow + sixteenbit(intensity_list[x])
			for x in range(fill):
				ow = ow + sixteenbit(intensity_list[q+x])
			f.writeframesraw(ow)
		else:
	...
	if superimpose:
		...
		for x in range(int(b)):
			ow += sixteenbit(glob_intensity_list[x])
		f.writeframesraw(ow)
